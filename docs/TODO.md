...
| Medium | Color contrast & visibility (light + dark) | Audit text/background contrast across **both light and dark themes**. In light mode, fix low-contrast cases such as the hardcoded `bg-*-50`/`text-*-800` pairs in `MotivationalBanner` and other dashboard panels. In dark mode, add the missing `dark:` variants for those same elements, since dark mode in `src/index.css` only restyles `.card` and `.input-field`. Verify WCAG AA contrast in both themes. |
